...
# NOTA: SHAP È STATO RIMOSSO DA QUI PER EVITARE IL CRASH DI TENSORFLOW
...
st.sidebar.markdown("---")
st.sidebar.info("STX Ultimate v6.6\nStable XAI Edition (ITA)")
...
if app_mode == "🔎 Analisi Singola (Deep Dive)":
    ...
    def train_lstm_final(df_log): # Funzione rinominata per evitare conflitti di cache
    ...
    if ticker_input:
        ...
        if df_p is None:
            ...
            model, scaler, scaled, cols, X_train = train_lstm_final(df_l)
            ...
            try:
                # IMPORTAZIONE LOCALE PER PREVENIRE CONFLITTI TENSORFLOW
                import shap 
                ...
                # 3. KernelExplainer (Model Agnostic - Molto più stabile)
                ...
                # 5. Calcolo Valori
                with st.spinner("Analisi XAI in corso (potrebbe richiedere qualche secondo)..."):
                ...
                # FALLBACK SILENZIOSO: Se SHAP fallisce, usiamo la correlazione degli ultimi 90gg come proxy
                # Questo evita schermate rosse e fornisce comunque un'indicazione utile
            ...
            fig_shap.update_layout(title="Impatto Fattori (Analisi XAI)", height=300)
            ...
            st.markdown(f"""
            <div class="explanation-box">
                <b>💡 Trasparenza Decisionale (White Box):</b><br>
                {"<br>".join(txt)}<br>
                <br>
                <i>Il sistema analizza quali variabili macroeconomiche hanno pesato di più nella decisione dell'AI.</i>
            </div>
            """, unsafe_allow_html=True)
